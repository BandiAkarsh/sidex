...
export function createSuite<T extends IStorageService>(params: {
	...
	test('Storage change source', () => {
		...
		storageService.storeAll(
			[{ key: 'testExternalChange', value: 'foobar', scope: StorageScope.WORKSPACE, target: StorageTarget.MACHINE }],
			true
		);
		...
		// Default source
		storageService.storeAll(
			[{ key: 'testChange', value: 'barfoo', scope: StorageScope.WORKSPACE, target: StorageTarget.MACHINE }],
			false
		);
