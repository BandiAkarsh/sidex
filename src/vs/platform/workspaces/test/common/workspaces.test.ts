...
suite('Workspaces', () => {
	test('reviveIdentifier', () => {
		...
		assert.strictEqual(
			isSingleFolderWorkspaceIdentifier(reviveIdentifier(serializedSingleFolderWorkspaceIdentifier)),
			true
		);
	...
	test('toWorkspaceIdentifier', () => {
		...
		identifier = toWorkspaceIdentifier({
			id: 'id',
			folders: [{ index: 0, name: 'test', toResource: () => URI.file('test'), uri: URI.file('test') }]
		});
		assert.ok(identifier);
