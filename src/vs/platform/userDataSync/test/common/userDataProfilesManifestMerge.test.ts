...
suite('UserDataProfilesManifestMerge', () => {
	...
	test('merge when there are no remote changes', () => {
		const localProfiles: IUserDataProfile[] = [toUserDataProfile('1', '1', URI.file('1'), URI.file('cache'))];
		const base: ISyncUserDataProfile[] = [{ id: '1', name: '1', collection: '1' }];
		const remoteProfiles: ISyncUserDataProfile[] = [{ id: '1', name: 'name changed', collection: '1' }];
