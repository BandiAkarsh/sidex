...
suite('ConfigurationRegistry', () => {
	...
	test('configuration defaults - deregister merged object default language overrides', async () => {
		...
		const overrides1 = [
			{ overrides: { '[lang]': { config: { a: 1, b: 2 } } }, source: { id: 'source1', displayName: 'source1' } }
		];
		const overrides2 = [
			{ overrides: { '[lang]': { config: { a: 2, c: 3 } } }, source: { id: 'source2', displayName: 'source2' } }
		];
	...
	test('configuration defaults - string source', async () => {
		...
		assert.deepStrictEqual(configurationRegistry.getConfigurationProperties()['config'].default, { a: 2, b: 2, c: 3 });
		assert.deepStrictEqual(
			configurationRegistry.getConfigurationProperties()['config'].defaultValueSource instanceof Map,
			true
		);
	...
	test('configuration defaults - deregister with string source and extension source', async () => {
		...
		const overrides2 = [
			{ overrides: { config: { a: 2, c: 3 } }, source: { id: 'extSource', displayName: 'Extension Source' } }
		];
