...
suite('TelemetryService', () => {
	...
	test(
		...
		sinonTestFn(function (this: any) {
			...
			mainWindow.onerror(
				'dangerousFilename',
				settings.dangerousPathWithImportantInfo.replace(settings.personalInfo, personInfoWithSpaces) + '/test.js',
				2,
				42,
			...
			assert.strictEqual(
				testAppender.events[0].data.file.indexOf(
					settings.dangerousPathWithImportantInfo.replace(settings.personalInfo, personInfoWithSpaces)
				),
				-1
			);
	...
	test(
		...
		sinonTestFn(function (this: any) {
			...
			// Test that important information remains but personal info does not
			assert.notStrictEqual(
				testAppender.events[0].data.callstack.indexOf(settings.anonymizedNodeModuleAsarPath),
				-1,
				'bare node_modules.asar path'
			);
			assert.notStrictEqual(
				testAppender.events[0].data.callstack.indexOf(settings.anonymizedNodeModulePath),
				-1,
	...
	test(
		...
		sinonTestFn(function (this: any) {
			...
			try {
				...
				assert.notStrictEqual(
					testAppender.events[0].data.callstack.indexOf(settings.extensionPathToRetain),
					-1,
					'User extension path should be retained'
				);
				assert.notStrictEqual(
					testAppender.events[0].data.callstack.indexOf(settings.anonymizedExtensionPath),
					-1,
				...
				assert.notStrictEqual(
					testAppender.events[0].data.callstack.indexOf(settings.serverInsidersExtensionPathToRetain),
					-1,
					'Server-insiders extension path should be retained'
				);
				assert.notStrictEqual(
					testAppender.events[0].data.callstack.indexOf(settings.anonymizedServerInsidersExtensionPath),
					-1,
				...
				assert.notStrictEqual(
					testAppender.events[0].data.callstack.indexOf(settings.builtinExtensionPathToRetain),
					-1,
					'Built-in extension path should be retained'
				);
				assert.notStrictEqual(
					testAppender.events[0].data.callstack.indexOf(settings.anonymizedBuiltinExtensionPath),
					-1,
					'Built-in extension path should be anonymized with preserved extension name'
				);
				// Verify the app path is removed
				assert.strictEqual(
					testAppender.events[0].data.callstack.indexOf('/Applications/Visual Studio Code.app'),
					-1,
