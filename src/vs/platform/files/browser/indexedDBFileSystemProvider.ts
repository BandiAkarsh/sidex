...
const ERR_UNKNOWN_INTERNAL = (message: string) =>
	createFileSystemProviderError(
		localize('internal', 'Internal error occurred in IndexedDB File System Provider. ({0})', message),
		FileSystemProviderErrorCode.Unknown
	);
...
export class IndexedDBFileSystemProvider extends Disposable implements IFileSystemProviderWithFileReadWriteCapability {
	...
	async readFile(resource: URI): Promise<Uint8Array> {
		...
		const buffer =
			result instanceof Uint8Array ? result : isString(result) ? VSBuffer.fromString(result).buffer : undefined;
		if (buffer === undefined) {
