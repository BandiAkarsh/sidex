...
suite('Auto Indent On Paste - TypeScript/JavaScript', () => {
	...
	test('issue #29753: incorrect indentation of header comment', () => {
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel, instantiationService) => {
			const text = ['/*----------------', ' *  Copyright (c) ', ' *  Licensed under ...', ' *-----------------*/'].join(
				'\n'
			);
			viewModel.paste(text, true, undefined, 'keyboard');
			const autoIndentOnPasteController = editor.registerAndInstantiateContribution(
	...
	test('issue #209859: do not do change indentation when pasted inside of a string', () => {
		...
		const initialText = ['const foo = "some text', '         which is strangely', '    indented"'].join('\n');
	...
	test.skip('issue #29589: incorrect indentation of closing brace on paste', () => {
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel, instantiationService) => {
			editor.setSelection(new Selection(2, 5, 2, 5));
			const text = ['function makeSub(a,b) {', 'subsent = sent.substring(a,b);', 'return subsent;', '}'].join('\n');
			const autoIndentOnPasteController = editor.registerAndInstantiateContribution(
				AutoIndentOnPaste.ID,
				AutoIndentOnPaste
			);
			...
			assert.strictEqual(
				model.getValue(),
				['function makeSub(a,b) {', 'subsent = sent.substring(a,b);', 'return subsent;', '}'].join('\n')
			);
		});
	});
...
suite('Auto Indent On Type - Visual Basic', () => {
	...
	test('issue #118932: indent after Module declaration', () => {
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			...
			viewModel.type('\n', 'keyboard');
			assert.strictEqual(model.getValue(), ['Module Test', '    '].join('\n'));
		});
	});
	...
	test('issue #118932: indent after Sub declaration', () => {
		...
		const model = createTextModel(['Module Test', '    '].join('\n'), languageId, {});
		disposables.add(model);
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			...
			viewModel.type('\n', 'keyboard');
			assert.strictEqual(model.getValue(), ['Module Test', '    Sub Main()', '        '].join('\n'));
		});
	});
	...
	test('issue #118932: dedent on End Sub', () => {
		...
		const model = createTextModel(
			['Module Test', '    Sub Main()', '        Console.WriteLine("Hello")', '        End Su'].join('\n'),
			languageId,
			{}
		);
		disposables.add(model);
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			...
			assert.strictEqual(
				model.getValue(),
				['Module Test', '    Sub Main()', '        Console.WriteLine("Hello")', '    End Sub'].join('\n')
			);
		});
	});
	...
	test('issue #118932: dedent on End Module', () => {
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			...
			assert.strictEqual(model.getValue(), ['Module Test', '    Private x As Integer', 'End Module'].join('\n'));
		});
	});
	...
	test('issue #118932: indent after Function declaration', () => {
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			...
			assert.strictEqual(
				model.getValue(),
				['Module Test', '    Function Add(a As Integer, b As Integer) As Integer', '        '].join('\n')
			);
		});
	});
	...
	test('issue #118932: dedent on End Function', () => {
		...
		const model = createTextModel(
			['Module Test', '    Function Add(a, b)', '        Return a + b', '        End Functio'].join('\n'),
			languageId,
			{}
		);
		disposables.add(model);
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			...
			assert.strictEqual(
				model.getValue(),
				['Module Test', '    Function Add(a, b)', '        Return a + b', '    End Function'].join('\n')
			);
		});
	});
	...
	test('issue #118932: indent after If Then', () => {
		...
		const model = createTextModel(['Sub Test()', '    '].join('\n'), languageId, {});
		disposables.add(model);
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			...
			viewModel.type('\n', 'keyboard');
			assert.strictEqual(model.getValue(), ['Sub Test()', '    If x > 0 Then', '        '].join('\n'));
		});
	});
	...
	test('issue #118932: indent after ElseIf Then', () => {
		...
		const model = createTextModel(
			['Sub Test()', '    If x > 0 Then', '        DoSomething()', '    ElseIf x < 0 Then'].join('\n'),
			languageId,
			{}
		);
		disposables.add(model);
	...
	test('issue #118932: dedent and indent on Else', () => {
		...
		const model = createTextModel(
			['Sub Test()', '    If x > 0 Then', '        DoSomething()', '        Els'].join('\n'),
			languageId,
			{}
		);
		disposables.add(model);
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			...
			assert.strictEqual(
				model.getValue(),
				['Sub Test()', '    If x > 0 Then', '        DoSomething()', '    Else'].join('\n')
			);
		});
	});
	...
	test('issue #118932: indent after While', () => {
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			...
			viewModel.type('\n', 'keyboard');
			assert.strictEqual(model.getValue(), ['Sub Test()', '    While x > 0', '        '].join('\n'));
		});
	});
	...
	test('issue #118932: dedent on End While', () => {
		...
		const model = createTextModel(
			['Sub Test()', '    While x > 0', '        x = x - 1', '        End Whil'].join('\n'),
			languageId,
			{}
		);
		disposables.add(model);
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			...
			assert.strictEqual(
				model.getValue(),
				['Sub Test()', '    While x > 0', '        x = x - 1', '    End While'].join('\n')
			);
		});
	});
	...
	test('issue #118932: indent after For', () => {
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			...
			viewModel.type('\n', 'keyboard');
			assert.strictEqual(model.getValue(), ['Sub Test()', '    For i = 1 To 10', '        '].join('\n'));
		});
	});
	...
	test('issue #118932: dedent on Next', () => {
		...
		const model = createTextModel(
			['Sub Test()', '    For i = 1 To 10', '        DoSomething(i)', '        Nex'].join('\n'),
			languageId,
			{}
		);
		disposables.add(model);
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			...
			assert.strictEqual(
				model.getValue(),
				['Sub Test()', '    For i = 1 To 10', '        DoSomething(i)', '    Next'].join('\n')
			);
		});
	});
	...
	test('issue #118932: indent after Do', () => {
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			...
			viewModel.type('Do');
			viewModel.type('\n', 'keyboard');
			assert.strictEqual(model.getValue(), ['Sub Test()', '    Do', '        '].join('\n'));
		});
	});
	...
	test('issue #118932: dedent on Loop', () => {
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			...
			assert.strictEqual(model.getValue(), ['Sub Test()', '    Do', '        x = x + 1', '    Loop'].join('\n'));
		});
	});
	...
	test('issue #118932: indent after Select Case', () => {
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			...
			viewModel.type('\n', 'keyboard');
			assert.strictEqual(model.getValue(), ['Sub Test()', '    Select Case x', '        '].join('\n'));
		});
	});
	...
	test('issue #118932: dedent on End Select', () => {
		...
		const model = createTextModel(['Sub Test()', '    Select Case x', '        End Selec'].join('\n'), languageId, {});
		disposables.add(model);

		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			editor.setSelection(new Selection(3, 18, 3, 18));
			viewModel.type('t', 'keyboard');
			assert.strictEqual(model.getValue(), ['Sub Test()', '    Select Case x', '    End Select'].join('\n'));
		});
	});
	...
	test('issue #118932: indent after Try', () => {
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			...
			viewModel.type('Try');
			viewModel.type('\n', 'keyboard');
			assert.strictEqual(model.getValue(), ['Sub Test()', '    Try', '        '].join('\n'));
		});
	});
	...
	test('issue #118932: dedent and indent on Catch', () => {
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			...
			assert.strictEqual(model.getValue(), ['Sub Test()', '    Try', '        DoSomething()', '    Catch'].join('\n'));
		});
	});
	...
	test('issue #118932: dedent and indent on Finally', () => {
		...
		const model = createTextModel(
			['Sub Test()', '    Try', '        DoSomething()', '    Catch', '        HandleError()', '        Finall'].join(
				'\n'
			),
			languageId,
			{}
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			...
			assert.strictEqual(
				model.getValue(),
				['Sub Test()', '    Try', '        DoSomething()', '    Catch', '        HandleError()', '    Finally'].join(
					'\n'
				)
			);
		});
	...
	test('issue #118932: indent after Class', () => {
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			...
			viewModel.type('\n', 'keyboard');
			assert.strictEqual(model.getValue(), ['Class MyClass', '    '].join('\n'));
		});
	});
	...
	test('issue #118932: dedent on End Class', () => {
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			...
			assert.strictEqual(model.getValue(), ['Class MyClass', '    Private x As Integer', 'End Class'].join('\n'));
		});
	});
	...
	test('issue #118932: full program indentation flow', () => {
		...
		withTestCodeEditor(model, { autoIndent: 'full', serviceCollection }, (editor, viewModel) => {
			...
			assert.strictEqual(
				model.getValue(),
				['Module Test', '    Sub Main()', '        Console.WriteLine("Hello, World!")', '        '].join('\n'),
			...
			assert.strictEqual(
				model.getValue(),
				['Module Test', '    Sub Main()', '        Console.WriteLine("Hello, World!")', '    End Sub'].join('\n'),
				'After End Sub'
			);
