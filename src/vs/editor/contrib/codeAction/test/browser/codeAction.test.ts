	...
	test('CodeActions are sorted by type, #38623', async () => {
		...
		const { validActions: actions } = disposables.add(
			await getCodeActions(
				...
				model,
				new Range(1, 1, 2, 1),
				{ type: languages.CodeActionTriggerType.Invoke, triggerAction: CodeActionTriggerSource.Default },
				Progress.None,
...

		{
			const { validActions: actions } = disposables.add(
				await getCodeActions(
					...
					model,
					new Range(1, 1, 2, 1),
					{ type: languages.CodeActionTriggerType.Auto, triggerAction: CodeActionTriggerSource.SourceAction },
					Progress.None,
					CancellationToken.None
				)
			);
