...
suite('OutlineModel', function () {
	...
	test('OutlineElement - updateMarker, 2', function () {
		...
		data = [fakeMarker(new Range(2, 4, 5, 4)), fakeMarker(new Range(2, 6, 2, 8)), fakeMarker(new Range(7, 6, 7, 8))];
		group.updateMarker(data);
		assert.strictEqual(p.marker!.count, 0);
	...
	test('OutlineElement - updateMarker/multiple groups', function () {
		...
		model.children
			...
			.children.set(
				'c1',
				new OutlineElement('c1', model.children.get('g1')!, fakeSymbolInformation(new Range(1, 1, 11, 1)))
			);
		...
		model.children
			...
			.children.set(
				'c2',
				new OutlineElement('c2', model.children.get('g2')!, fakeSymbolInformation(new Range(1, 1, 7, 1)))
			);
		model.children
			.get('g2')!
			.children.get('c2')!
