...
suite('Inline Completions', () => {
	...
	test('Ghost text is shown after trigger', async function () {
		...
		await withAsyncTestCodeEditorAndInlineCompletionsModel(
			...
			async ({ editor, editorViewModel, model, context }) => {
				...
				assert.deepStrictEqual(provider.getAndClearCallHistory(), [{ position: '(1,4)', text: 'foo', triggerKind: 1 }]);
	...
	test('Ghost text is shown automatically when configured', async function () {
		...
		await withAsyncTestCodeEditorAndInlineCompletionsModel(
			...
			async ({ editor, editorViewModel, model, context }) => {
				...
				assert.deepStrictEqual(provider.getAndClearCallHistory(), [{ position: '(1,4)', text: 'foo', triggerKind: 0 }]);
				assert.deepStrictEqual(context.getAndClearViewStates(), ['', 'foo[bar]']);
			}
		);
	});
	...
	test('Unindent whitespace', async function () {
		...
		await withAsyncTestCodeEditorAndInlineCompletionsModel(
			...
			async ({ editor, editorViewModel, model, context }) => {
				...
				assert.deepStrictEqual(provider.getAndClearCallHistory(), [{ position: '(1,3)', text: '  ', triggerKind: 1 }]);
	...
	suite('Forward Stability', () => {
		...
		async function setupScenario(
			{ editor, editorViewModel, model, context, store }: IWithAsyncTestCodeEditorAndInlineCompletionsModel,
		...
		test('Support forward instability', async function () {
			...
			await withAsyncTestCodeEditorAndInlineCompletionsModel('', { fakeClock: true, provider }, async ctx => {
				...
				assert.deepStrictEqual(provider.getAndClearCallHistory(), [{ position: '(1,3)', text: 'fo', triggerKind: 0 }]);
	...
	test('Multi Part', async function () {
		...
		await withAsyncTestCodeEditorAndInlineCompletionsModel(
			...
			async ({ editor, editorViewModel, model, context }) => {
				...
				await timeout(1000);
				model.accept(editor);
				assert.deepStrictEqual(editor.getValue(), [`console.log("hello");`, `console.log`, ``].join('\n'));
			}
		);
	});
	...
	test('Multi Part and Different Cursor Columns', async function () {
		...
		await withAsyncTestCodeEditorAndInlineCompletionsModel(
			...
			async ({ editor, editorViewModel, model, context }) => {
				...
				await timeout(1000);
				model.accept(editor);
				assert.deepStrictEqual(editor.getValue(), [`console.log("hello");`, `console.warn`, ``].join('\n'));
			}
		);
	});
	...
	test('Partial Multi-Part Completion', async function () {
		...
		await withAsyncTestCodeEditorAndInlineCompletionsModel(
			...
			async ({ editor, editorViewModel, model, context }) => {
				...
				assert.deepStrictEqual(editor.getValue(), [`for (let i)`, `for `, ``].join('\n'));
			}
		);
	});
	...
	test('Partial Mutli-Part and Different Cursor Columns Completion', async function () {
		...
		await withAsyncTestCodeEditorAndInlineCompletionsModel(
			...
			async ({ editor, editorViewModel, model, context }) => {
				...
				assert.deepStrictEqual(editor.getValue(), [`console.log("hello" + )`, `console.warnnnn`, ``].join('\n'));
			}
		);
	});
