...
suite('SnippetController', () => {
	...
	function snippetTest(
		cb: (editor: ITestCodeEditor, template: string, snippetController: TestSnippetController) => void,
		lines?: string[]
	): void {
		if (!lines) {
	...
	test('Final tabstop with multiple selections', () => {
		...
		snippetTest((editor, codeSnippet, snippetController) => {
			...
			assert.ok(
				second.equalsRange({ startLineNumber: 1, startColumn: 14, endLineNumber: 1, endColumn: 14 }),
				second.toString()
			);
		});
		...
		snippetTest((editor, codeSnippet, snippetController) => {
			...
			assert.ok(
				editor.getSelection()!.equalsRange({ startLineNumber: 2, startColumn: 8, endColumn: 8, endLineNumber: 2 })
			);
		});
	});
	...
	test('Final tabstop, #11742 simple', () => {
		snippetTest(
			(editor, codeSnippet, controller) => {
				...
				assert.strictEqual(editor.getSelections()!.length, 1);
				assert.ok(
					editor.getSelection()!.equalsRange({ startLineNumber: 1, startColumn: 27, endLineNumber: 1, endColumn: 27 })
				);
		...
		snippetTest(
			(editor, codeSnippet, controller) => {
				...
				assert.strictEqual(editor.getSelections()!.length, 1);
				assert.ok(
					editor.getSelection()!.equalsRange({ startLineNumber: 2, startColumn: 2, endLineNumber: 2, endColumn: 2 }),
		...
		snippetTest(
			(editor, codeSnippet, controller) => {
				...
				assert.strictEqual(editor.getSelections()!.length, 1);
				assert.ok(
					editor.getSelection()!.equalsRange({ startLineNumber: 2, startColumn: 1, endLineNumber: 2, endColumn: 1 }),
					editor.getSelection()!.toString()
				);
				assert.strictEqual(editor.getModel()!.getValue(), 'afterEach((done) => {\n\ttest\n});');
			},
		...
		snippetTest(
			(editor, codeSnippet, controller) => {
				...
				assert.strictEqual(editor.getSelections()!.length, 1);
				assert.ok(
					editor.getSelection()!.equalsRange({ startLineNumber: 1, startColumn: 4, endLineNumber: 1, endColumn: 4 }),
					editor.getSelection()!.toString()
				);
			},
			['afterone']
	...
	test('Final tabstop, #11742 different indents', () => {
		snippetTest(
			(editor, codeSnippet, controller) => {
				...
				assert.ok(
					second.equalsRange({ startLineNumber: 2, startColumn: 2, endLineNumber: 2, endColumn: 2 }),
					second.toString()
				);
			},
			['af', '\taf']
	...
	test('Final tabstop, no tabstop', () => {
		snippetTest(
			(editor, codeSnippet, controller) => {
				...
				assert.ok(
					editor.getSelection()!.equalsRange({ startLineNumber: 1, startColumn: 10, endLineNumber: 1, endColumn: 10 })
				);
			},
			['af', '\taf']
		);
	...
	test('Multiple cursor and overwriteBefore/After, issue #11060', () => {
		...
		snippetTest(
			(editor, codeSnippet, controller) => {
				editor.setSelections([new Selection(1, 7, 1, 7), new Selection(2, 4, 2, 4), new Selection(3, 5, 3, 5)]);
