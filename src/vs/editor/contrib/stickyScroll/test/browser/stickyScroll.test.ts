...
suite('Sticky Scroll Tests', () => {
			...
			await withAsyncTestCodeEditor(
				...
				async (editor, _viewModel, instantiationService) => {
					...
					assert.deepStrictEqual(
						...
						[new StickyLineCandidate(7, 11, 0, 19), new StickyLineCandidate(9, 11, 19, 19)]
					);
					assert.deepStrictEqual(
						provider.getCandidateStickyLinesIntersecting({ startLineNumber: 10, endLineNumber: 13 }),
						[new StickyLineCandidate(7, 11, 0, 19), new StickyLineCandidate(9, 11, 19, 19)]
					);
	...
	const textWithScopesWithSameStartingLines = ['class TestClass { foo() {', 'function bar(){', '', '}}', '}', ''].join(
		'\n'
	);
