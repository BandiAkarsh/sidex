			...
			this._notebookService.registerNotebookSerializer(viewType, extension, {
				...
				searchInNotebooks: async (
					...
					const includes = fileNames.map(selector => {
						const globPattern =
							(selector as INotebookExclusiveDocumentFilter).include || (selector as IRelativePattern | string);
...
CommandsRegistry.registerCommand(
	...
	async (accessor, notebookType: string, dto: SerializableObjectWithBuffers<NotebookDataDto>) => {
		assertType(typeof notebookType === 'string', 'string');
