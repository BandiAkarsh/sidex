...
export class MainThreadNotebookDocuments implements MainThreadNotebookDocumentsShape {
	...
	constructor(
		...
		this._disposables.add(
			this._notebookEditorModelResolverService.onDidSaveNotebook(e => this._proxy.$acceptModelSaved(e))
		);
							...
							case NotebookCellsChangeType.ModelChange:
								eventDto.rawEvents.push({
									...
									changes: e.changes.map(
										diff =>
											[diff[0], diff[1], diff[2].map(cell => NotebookDto.toNotebookCellDto(cell))] as [
												number,
												number,
												NotebookCellDto[]
											]
