...
export class ExtHostTunnelService extends Disposable implements IExtHostTunnelService {
	...
	private _extensionTunnels: Map<string, Map<number, { tunnel: vscode.Tunnel; disposeListener: IDisposable }>> =
		new Map();
	...
	async $applyCandidateFilter(candidates: CandidatePort[]): Promise<CandidatePort[]> {
		const filter = await Promise.all(
			candidates.map(candidate => this._showCandidatePort(candidate.host, candidate.port, candidate.detail ?? ''))
		);
