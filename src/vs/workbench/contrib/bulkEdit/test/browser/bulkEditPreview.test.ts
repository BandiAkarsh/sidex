...
suite('BulkEditPreview', function () {
	...
	test('category selection', async function () {
		const edits = [
			...
			new ResourceTextEdit(URI.parse('some:///uri2'), { text: 'foo', range: new Range(1, 1, 1, 1) }, undefined, {
				label: 'C2',
				needsConfirmation: false
			})
		];
	...
	test('fix bad metadata', async function () {
		...
		const edits = [
			...
			new ResourceTextEdit(URI.parse('some:///uri1'), { text: 'foo', range: new Range(1, 1, 1, 1) }, undefined, {
				label: 'C2',
				needsConfirmation: false
			})
		];
