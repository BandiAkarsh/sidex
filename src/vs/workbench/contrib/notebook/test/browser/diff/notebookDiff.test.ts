	...
	test('diff output', async () => {
		await withTestNotebookDiffModel(
			...
			async (model, disposables, accessor) => {
				...
				diffViewModel.items[0].showHiddenCells();
				assert.strictEqual(
					(diffViewModel.items[0] as unknown as SideBySideDiffElementViewModel).checkIfOutputsModified(),
					false
				);
				assert.strictEqual(diffViewModel.items[1].type, 'modified');
	...
	test('diff output fast check', async () => {
		await withTestNotebookDiffModel(
			...
			async (model, disposables, accessor) => {
				...
				diffViewModel.items[0].showHiddenCells();
				assert.strictEqual(
					(diffViewModel.items[0] as unknown as SideBySideDiffElementViewModel).original!.textModel.equal(
						(diffViewModel.items[0] as unknown as SideBySideDiffElementViewModel).modified!.textModel
					),
					true
				);
				assert.strictEqual(
					(diffViewModel.items[1] as unknown as SideBySideDiffElementViewModel).original!.textModel.equal(
						(diffViewModel.items[1] as unknown as SideBySideDiffElementViewModel).modified!.textModel
					),
					false
				);
