...
suite('NotebookDiff Diff Service', () => {
		...
		const { mapping, diff } = await mapCells(
			[
				[
					...
					'javascript',
					CellKind.Code,
					[{ outputId: 'someOtherId', outputs: [{ mime: Mimes.text, data: VSBuffer.wrap(new Uint8Array([3])) }] }],
	...
	test('diff test data single cell', async () => {
		const { mapping, diff } = await mapCells(
			[[['# This version has a bug\n', 'def mult(a, b):\n', '    return a / b'], 'javascript', CellKind.Code, [], {}]],
			[
				[
		...
		const { mapping, diff } = await mapCells(
			[
				[
					...
					'javascript',
					CellKind.Code,
					[{ outputId: 'someId', outputs: [{ mime: Mimes.text, data: VSBuffer.wrap(new Uint8Array([6])) }] }],
				...
				[
					...
					'javascript',
					CellKind.Code,
					[{ outputId: 'someId', outputs: [{ mime: Mimes.text, data: VSBuffer.wrap(new Uint8Array([2])) }] }],
		...
		const { mapping, diff } = await mapCells(
			[
				...
				[
					...
					'javascript',
					CellKind.Code,
					[{ outputId: 'someId', outputs: [{ mime: Mimes.text, data: VSBuffer.wrap(new Uint8Array([3])) }] }],
		...
		const { mapping, diff } = await mapCells(
			[
				...
				[
					...
					'javascript',
					CellKind.Code,
					[{ outputId: 'someId', outputs: [{ mime: Mimes.text, data: VSBuffer.wrap(new Uint8Array([5])) }] }],
					{}
				]
			],
			[
		...
		const { mapping, diff } = await mapCells(
			[
				...
				[
					...
					'javascript',
					CellKind.Code,
					[{ outputId: 'someOtherId', outputs: [{ mime: Mimes.text, data: VSBuffer.wrap(new Uint8Array([4])) }] }],
					{ metadata: { collapsed: false }, executionOrder: 3 }
				]
			],
			[
				...
				[
					...
					'javascript',
					CellKind.Code,
					[{ outputId: 'someOtherId', outputs: [{ mime: Mimes.text, data: VSBuffer.wrap(new Uint8Array([5])) }] }],
					{ metadata: { collapsed: false }, executionOrder: 3 }
				]
			]
		);
	...
	test('Detect modification and insertion of cells', async () => {
		const { mapping, diff } = await mapCells(
			...
			[
				...
				{
					cell_type: 'markdown',
					source: ['### Convert Model to OpenVINO IR format\n', '[back to top ⬆️](#Table-of-contents:)']
				},
				{
					cell_type: 'code',
					source: [
	...
	test('Modification with Insertion, detected as deletion of a cell', async () => {
		const { mapping, diff } = await mapCells(
			[
				...
				{
					cell_type: 'markdown',
					source: ['## Install and import required packages\n', '[back to top ⬆️](#Table-of-contents:)\n']
				},
				{
					cell_type: 'markdown',
					source: [
				...
				{
					cell_type: 'code',
					source: ['pipe = diffusers.DiffusionPipeline.from_pretrained("cerspense/zeroscope_v2_576w")']
				},
				{
					cell_type: 'code',
					source: [
				...
				{
					cell_type: 'markdown',
					source: ['### Define the conversion function\n', '[back to top ⬆️](#Table-of-contents:)\n']
				},
				{
					cell_type: 'markdown',
					source: [
				...
				{
					cell_type: 'markdown',
					source: ['Text encoder is used to encode the input prompt to tensor. Default tensor length is 77.']
				},
				{
					cell_type: 'code',
					source: [
				...
				{
					cell_type: 'code',
					source: ['ov_pipe = OVTextToVideoSDPipeline(ov_vae_decoder, ov_text_encoder, tokenizer, ov_unet, scheduler)']
				},
				{
					cell_type: 'markdown',
		...
		const { mapping, diff } = await mapCells(
			[
				...
				},
				{
					...
					source: ['Please select below whether you would like to run quantization to improve model inference speed.']
				},
				{
					cell_type: 'code',
				...
				},
				{
					...
					source: ["Let's load `skip magic` extension to skip quantization if `to_quantize` is not selected"]
				},
				{
					cell_type: 'code',
				...
				},
				{
					cell_type: 'markdown',
					id: 'b289f057',
					source: ['## Compare the Original and Quantized Models\n', '[back to top ⬆️](#Table-of-contents:)\n']
				},
				{
					cell_type: 'markdown',
		...
		const { mapping, diff } = await mapCells(
			[
				...
				},
				{
					...
					source: ['## Export PyTorch model to OpenVINO IR Format\n', '[back to top ⬆️](#Table-of-contents:)']
				},
				{
					cell_type: 'markdown',
				...
				},
				{
					...
					source: ['### Run Optimized Model on AUTO device\n', '[back to top ⬆️](#Table-of-contents:)']
				},
				{
					cell_type: 'code',
				...
				},
				{
					...
					source: ['### Run Optimized Model Inference on selected device\n', '[back to top ⬆️](#Table-of-contents:)']
				},
				{
					cell_type: 'code',
				...
				},
				{
					...
					source: ['!benchmark_app -m $ov_model_path -d $device.value -api async -shape "[1,3,640,640]" -t 15']
				},
				{
					cell_type: 'markdown',
		...
		const { mapping, diff } = await mapCells(
			[
				...
				},
				{
					...
					source: ['**NOTE:** Execute cell below and use your mouse to draw bounding box on the image 👇']
				},
				{
					cell_type: 'code',
					execution_count: null,
					metadata: {
						colab: {
							base_uri: 'https://localhost:8080/',
							height: 1000,
							referenced_widgets: ['a747e095e21448f29d8697fb6b1a7a74', '6401ca8c2f574ba5ab62f0f06b9e0d06']
						},
						id: 'nLA7xmDIMTfx',
	...
	async function mapCells(original: MockNotebookCell[], modified: MockNotebookCell[]) {
		const mapping = matchCellBasedOnSimilarties(modified.map(cellToDto), original.map(cellToDto)).map(mapping => ({
