...
abstract class KernelPickerStrategyBase implements IKernelPickerStrategy {
	...
	async showQuickPick(editor: IActiveNotebookEditor, wantedId?: string, skipAutoRun?: boolean): Promise<boolean> {
		...
		const extensionRecommendataionPromise =
			quickPickItems.length === 0
				? createCancelablePromise(token =>
						this._showInstallKernelExtensionRecommendation(notebook, quickPick, this._extensionWorkbenchService, token)
					)
				: undefined;
		...
		const kernelChangeEventListener = Event.debounce<void, void>(
			...
			for (const item of currentActiveItems) {
				if (isKernelPick(item)) {
					...
					const sameItem = quickPickItems.find(pi => isKernelPick(pi) && pi.kernel.id === kernelId) as
						| KernelPick
						| undefined;
					if (sameItem) {
		...
		const pick = await new Promise<{ selected: KernelQuickPickItem | undefined; items: KernelQuickPickItem[] }>(
			(resolve, reject) => {
	...
	protected async _showKernelExtension(
		...
		for (const extId of extIds) {
			...
			if (
				...
				!extensionWorkbenchService.installed.some(e => areSameExtensions(e.identifier, extension.identifier))
			) {
	...
	protected async _getKernelRecommendationsQuickPickItems(
		...
		if (suggestedExtension) {
			...
			quickPickItems.push({
				...
				label:
					`$(${Codicon.lightbulb.id}) ` + localize('installSuggestedKernel', 'Install/Enable suggested extensions'),
	...
	private getSuggestedLanguage(notebookTextModel: NotebookTextModel): string | undefined {
		...
		const language_info = (metaData?.metadata as Record<string, unknown>)?.language_info as
				...
				quickPick.onDidTriggerItemButton(async e => {
					if (isKernelSourceQuickPickItem(e.item) && e.item.documentation !== undefined) {
						const uri = URI.isUri(e.item.documentation)
							...
							: await this._commandService.executeCommand<URI>(e.item.documentation);
						if (uri) {
