...
registerAction2(
	class extends NotebookCellAction {
		...
		async runWithContext(accessor: ServicesAccessor, context: INotebookCellActionContext) {
			...
			const splitPoints =
				cell.focusMode === CellFocusMode.Container ? [{ lineNumber: 1, column: 1 }] : cell.getSelectionsStartPosition();
		...
		private toggleOutputScrolling(viewModel: ICellOutputViewModel, globalScrollSetting: boolean, collapsed: boolean) {
			...
			if (cellMetadata) {
				const currentlyEnabled =
					cellMetadata['scrollable'] !== undefined ? cellMetadata['scrollable'] : globalScrollSetting;
