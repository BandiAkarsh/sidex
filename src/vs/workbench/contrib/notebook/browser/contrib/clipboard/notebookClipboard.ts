...
export function runCutCells(
	...
	if (targetCell) {
		...
		if (!containingSelection) {
			...
			const newSelections = selections.map(selection =>
				selection.end <= targetCellIndex ? selection : { start: selection.start - 1, end: selection.end - 1 }
			);
	...
	if (!containingSelection) {
		...
		const newSelections = selections.map(selection =>
			selection.end <= focus.start ? selection : { start: selection.start - 1, end: selection.end - 1 }
		);
		textModel.applyEdits(
	...
	const newFocusedCellIndex =
		firstSelectIndex < textModel.cells.length - 1 ? firstSelectIndex : Math.max(textModel.cells.length - 2, 0);
