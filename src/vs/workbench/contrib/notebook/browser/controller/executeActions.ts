...
function handleAutoReveal(cell: ICellViewModel, notebookEditor: IActiveNotebookEditor): void {
	...
	const isFullyVisible =
		cellEditorScrollTop >= notebookEditor.scrollTop && cellOutputScrollBottom <= notebookEditor.scrollBottom;
	const isEditorBottomVisible =
		cellEditorScrollBottom - 25 /* padding for the cell status bar */ >= notebookEditor.scrollTop &&
		cellEditorScrollBottom + 25 /* padding to see a sliver of the beginning of outputs */ <=
	...
	class ExecuteCellSelectBelow extends NotebookCellAction {
		...
		async runWithContext(accessor: ServicesAccessor, context: INotebookCellActionContext): Promise<void> {
			...
			if (scrollBehavior === 'none') {
				...
				focusOptions = {
					revealBehavior:
						scrollBehavior === 'fullCell' ? ScrollToRevealBehavior.fullCell : ScrollToRevealBehavior.firstLine
				};
			}
