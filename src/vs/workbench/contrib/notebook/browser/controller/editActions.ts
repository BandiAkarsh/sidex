	...
	class ChangeCellLanguageAction extends NotebookCellAction<ICellRange> {
		...
		private async showLanguagePicker(accessor: ServicesAccessor, context: IChangeCellContext) {
			...
			if (!languages) {
				...
				languages =
					allSupportedLanguages.length > 0 ? allSupportedLanguages : languageService.getRegisteredLanguageIds();
			}
			...
			providerLanguages.forEach(languageId => {
				...
				if (
					context.cell.cellKind === CellKind.Markup ? languageId === 'markdown' : languageId === context.cell.language
				) {
