	...
	items: {
		...
		allOf: [
			...
			},
			{
				oneOf: [
					{
						required: [NotebookRendererContribution.entrypoint, NotebookRendererContribution.mimeTypes],
						properties: {
...
class NotebookRenderersDataRenderer extends Disposable implements IExtensionFeatureTableRenderer {
	...
	render(manifest: IExtensionManifest): IRenderedData<ITableData> {
		...
		const headers = [nls.localize('Notebook renderer name', 'Name'), nls.localize('Notebook mimetypes', 'Mimetypes')];
