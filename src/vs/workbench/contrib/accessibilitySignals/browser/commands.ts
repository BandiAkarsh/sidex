...
export class ShowSignalSoundHelp extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		const items: (IQuickPickItem & { signal: AccessibilitySignal })[] = AccessibilitySignal.allAccessibilitySignals
			.map((signal, idx) => ({
		...
		disposables.add(
			qp.onDidAccept(() => {
				...
				for (const signal of enabledSounds) {
					let { sound, announcement } = configurationService.getValue<{ sound: string; announcement?: string }>(
						signal.settingsKey
					);
