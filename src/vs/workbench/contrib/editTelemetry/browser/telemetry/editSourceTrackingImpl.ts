...
export class EditSourceTrackingImpl extends Disposable {
	...
	constructor(
		...
		this._states = mapObservableArrayCached(this, this._annotatedDocuments.documents, (doc, store) => {
			return [
				doc.document,
				store.add(this._instantiationService.createInstance(TrackedDocumentInfo, doc, scmBridge, this._statsEnabled))
			] as const;
		});
...
class TrackedDocumentInfo extends Disposable {
	...
	constructor(
		...
		this.windowedTracker = derived(reader => {
			...
			reader.store.add(
				this._userAttentionService.fireAfterGivenFocusTimePassed(10 * 60 * 1000, () => {
		...
		this.windowedFocusTracker = derived(reader => {
			...
			reader.store.add(
				this._userAttentionService.fireAfterGivenFocusTimePassed(20 * 60 * 1000, () => {
