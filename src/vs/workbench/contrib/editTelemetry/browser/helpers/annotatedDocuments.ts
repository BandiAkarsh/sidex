	...
	constructor(
		...
		this._states = mapObservableArrayCached(this, this._workspace.documents, (doc, store) => {
			...
			return wasEverVisible.map(v =>
				v ? store.add(this._instantiationService.createInstance(AnnotatedDocument, doc, docIsVisible)) : undefined
			);
		});
...
export class UriVisibilityProvider {
	...
	constructor(@IEditorGroupsService private readonly _editorGroupsService: IEditorGroupsService) {
...
export class AnnotatedDocument extends Disposable {
	...
	constructor(
		...
		processedDoc = this._store.add(
			this._instantiationService.createInstance(CombineStreamedChanges<EditSourceData>, processedDoc)
		);
