...
export class OutlinePane extends ViewPane implements IOutlinePane {
	...
	private async _handleEditorControlChanged(pane: IEditorPane | undefined): Promise<void> {
		...
		this._editorControlDisposables.add(
			tree.onDidChangeFindMode(mode => (this._outlineViewState.filterOnType = mode === TreeFindMode.Filter))
		);
		...
		this._editorControlDisposables.add(
			this._outlineViewState.onDidChange((e: { followCursor?: boolean; sortBy?: boolean; filterOnType?: boolean }) => {
