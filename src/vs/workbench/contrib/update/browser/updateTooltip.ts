...
export class UpdateTooltip extends Disposable {
	...
	private renderIdle({ error, notAvailable }: Idle) {
		...
		if (notAvailable) {
			...
			this.renderMessage(
				localize('updateTooltip.noUpdateAvailableMessage', 'There are no updates currently available.'),
				Codicon.info
			);
			return;
		}
		...
		switch (this.configurationService.getValue<string>('update.mode')) {
			case 'none':
				this.renderMessage(
					localize('updateTooltip.autoUpdateNone', 'Automatic updates are disabled.'),
					Codicon.warning
				);
				break;
			case 'manual':
	...
	private renderOverwriting({ update }: Overwriting) {
		...
		this.renderMessage(
			localize('updateTooltip.downloadingNewerPleaseWait', 'A newer update was released. Downloading, please wait...')
		);
	}

	private renderRestarting({ update }: Restarting) {
		this.renderTitleAndInfo(
			localize('updateTooltip.restartingTitle', 'Restarting {0}', this.productService.nameShort),
			update
		);
