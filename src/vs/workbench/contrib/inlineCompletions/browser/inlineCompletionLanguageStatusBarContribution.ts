			...
			autorunWithStore((reader, store) => {
				...
				const statusMap: Record<typeof status, { shortLabel: string; label: string; loading: boolean }> = {
					...
					ghostText: {
						...
						label: '$(lightbulb) ' + localize('inlineCompletionAvailable', 'Inline completion available'),
						loading: false
					},
					inlineEdit: {
						shortLabel: '$(lightbulb-sparkle)',
						label: '$(lightbulb-sparkle) ' + localize('inlineEditAvailable', 'Inline edit available'),
						loading: false
					},
					noSuggestion: {
						shortLabel: '$(circle-slash)',
						label: '$(circle-slash) ' + localize('noInlineSuggestionAvailable', 'No inline suggestion available'),
						loading: false
					}
				};
