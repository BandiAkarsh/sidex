...
export class WalkthroughsService extends Disposable implements IWalkthroughsService {
									...
									: step.media.type === 'markdown'
										...
										: {
												...
												path: convertRelativeMediaPathsToWebviewURIs(
													FileAccess.asFileUri('vs/workbench/contrib/welcomeGettingStarted/common/media/'),
													step.media.path
												),
												altText: step.media.altText,
												root: FileAccess.asFileUri('vs/workbench/contrib/welcomeGettingStarted/common/media/'),
	...
	private initCompletionEventListeners() {
		this._register(
			this.commandService.onDidExecuteCommand(command => this.progressByEvent(`onCommand:${command.commandId}`))
		);
	...
	private resolveWalkthrough(category: IWalkthrough): IResolvedWalkthrough {
		...
		const hasNewSteps =
			lastStepIDs &&
			(currentStepIds.length !== lastStepIDs.length || currentStepIds.some((id, index) => id !== lastStepIDs[index]));
						...
						linkedText.nodes
							...
							.map(({ href }) => {
								if (href.startsWith('command:')) {
									return (
										'onCommand:' + href.slice('command:'.length, href.includes('?') ? href.indexOf('?') : undefined)
									);
								}
