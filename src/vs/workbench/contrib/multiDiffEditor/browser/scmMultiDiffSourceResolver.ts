...
export class ScmHistoryItemResolver implements IMultiDiffSourceResolver {
	...
	async resolveDiffSource(uri: URI): Promise<IResolvedMultiDiffSource> {
		const { repositoryId, historyItemId, historyItemParentId, historyItemDisplayId } =
...
class ScmResolvedMultiDiffSource implements IResolvedMultiDiffSource {
	...
	constructor(
		...
		this._resources = observableFromEvent<MultiDiffEditorItem[]>(this._group.onDidChangeResources, () =>
