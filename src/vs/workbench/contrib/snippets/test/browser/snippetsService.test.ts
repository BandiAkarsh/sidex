...
suite('SnippetsService', function () {
	...
	setup(function () {
		...
		snippetService = new SimpleSnippetService([
			new Snippet(false, ['fooLang'], 'barTest', 'bar', '', 'barCodeSnippet', '', SnippetSource.User, generateUuid()),
			new Snippet(false, ['fooLang'], 'bazzTest', 'bazz', '', 'bazzCodeSnippet', '', SnippetSource.User, generateUuid())
		]);
	});
	...
	test('snippet completions - with different prefixes', async function () {
		snippetService = new SimpleSnippetService([
			new Snippet(false, ['fooLang'], 'barTest', 'bar', '', 's1', '', SnippetSource.User, generateUuid()),
			new Snippet(false, ['fooLang'], 'name', 'bar-bar', '', 's2', '', SnippetSource.User, generateUuid())
		]);
	...
	test('Cannot use "<?php" as user snippet prefix anymore, #26275', async function () {
		snippetService = new SimpleSnippetService([
			new Snippet(false, ['fooLang'], '', '<?php', '', 'insert me', '', SnippetSource.User, generateUuid())
		]);
	...
	test('No user snippets in suggestions, when inside the code, #30508', async function () {
		snippetService = new SimpleSnippetService([
			new Snippet(false, ['fooLang'], '', 'foo', '', '<foo>$0</foo>', '', SnippetSource.User, generateUuid())
		]);
	...
	test('SnippetSuggest - ensure extension snippets come last ', async function () {
		snippetService = new SimpleSnippetService([
			new Snippet(false, ['fooLang'], 'second', 'second', '', 'second', '', SnippetSource.Extension, generateUuid()),
			new Snippet(false, ['fooLang'], 'first', 'first', '', 'first', '', SnippetSource.User, generateUuid())
		]);
	...
	test('Dash in snippets prefix broken #53945', async function () {
		snippetService = new SimpleSnippetService([
			new Snippet(false, ['fooLang'], 'p-a', 'p-a', '', 'second', '', SnippetSource.User, generateUuid())
		]);
		const provider = new SnippetCompletionProvider(
			languageService,
			snippetService,
	...
	test('No snippets suggestion on long lines beyond character 100 #58807', async function () {
		snippetService = new SimpleSnippetService([
			new Snippet(false, ['fooLang'], 'bug', 'bug', '', 'second', '', SnippetSource.User, generateUuid())
		]);
	...
	test("substring of prefix can't trigger snippet #60737", async function () {
		snippetService = new SimpleSnippetService([
			new Snippet(false, ['fooLang'], 'mytemplate', 'mytemplate', '', 'second', '', SnippetSource.User, generateUuid())
		]);
	...
	test('issue #61296: VS code freezes when editing CSS fi`le with emoji', async function () {
		...
		snippetService = new SimpleSnippetService([
			new Snippet(false, ['fooLang'], 'bug', '-a-bug', '', 'second', '', SnippetSource.User, generateUuid())
		]);
	...
	test('Snippet prefix with special chars and numbers does not work #62906', async function () {
		snippetService = new SimpleSnippetService([
			new Snippet(false, ['fooLang'], 'noblockwdelay', '<<', '', '<= #dly"', '', SnippetSource.User, generateUuid()),
			new Snippet(false, ['fooLang'], 'noblockwdelay', '11', '', 'eleven', '', SnippetSource.User, generateUuid())
		]);
	...
	test('Snippet replace-range incorrect #108894', async function () {
		snippetService = new SimpleSnippetService([
			new Snippet(false, ['fooLang'], 'eng', 'eng', '', '<span></span>', '', SnippetSource.User, generateUuid())
		]);
