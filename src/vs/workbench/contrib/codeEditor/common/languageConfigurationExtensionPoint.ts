	...
	private async _readConfigFile(configFileLocation: URI): Promise<ILanguageConfiguration> {
		try {
			...
			if (getNodeType(configuration) !== 'object') {
				console.error(
					nls.localize('formatError', '{0}: Invalid format, JSON object expected.', configFileLocation.toString())
				);
				configuration = {};
			}
		...
		if (configuration.folding) {
			...
			const startMarker =
				rawMarkers && rawMarkers.start
					...
					: undefined;
			const endMarker =
				rawMarkers && rawMarkers.end ? this._parseRegex(languageId, `folding.markers.end`, rawMarkers.end) : undefined;
