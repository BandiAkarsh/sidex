...
class ManageAccountPreferenceForExtensionActionImpl {
	...
	async run(extensionId?: string, providerId?: string) {
		...
		if (provider.supportsMultipleAccounts) {
			...
			const lastUsedScopes = accounts
				.flatMap(account =>
					this._authenticationQueryService.provider(providerId).account(account.label).extension(extensionId).getUsage()
				)
	...
	private _createQuickPick(
		...
		const picker = disposableStore.add(
			this._quickInputService.createQuickPick<AccountPreferenceQuickPickItem>({ useSeparators: true })
		);
		disposableStore.add(
			picker.onDidHide(() => {
