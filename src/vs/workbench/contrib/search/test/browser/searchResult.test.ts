...
suite('SearchResult', () => {
	...
	test('Test that notebook matches get added correctly', function () {
		...
		assert.deepStrictEqual(
			...
			(addFileMatch.getCall(0).args[0][0] as INotebookFileMatchWithModel).cellResults[0].contentResults
		);
		assert.deepStrictEqual(
			fileMatch1.cellResults[0].webviewResults,
			(addFileMatch.getCall(0).args[0][0] as INotebookFileMatchWithModel).cellResults[0].webviewResults
		);
		assert.deepStrictEqual(
			fileMatch2.cellResults[0].contentResults,
			(addFileMatch.getCall(0).args[0][1] as INotebookFileMatchWithModel).cellResults[0].contentResults
		);
		assert.deepStrictEqual(
			fileMatch2.cellResults[0].webviewResults,
			(addFileMatch.getCall(0).args[0][1] as INotebookFileMatchWithModel).cellResults[0].webviewResults
		);
	});
	...
	test('batchRemove should remove FolderMatchNoRoot (Other files) correctly', function () {
		...
		addToSearchResult(testObject, [aRawMatch('/workspace/file.txt', new TextSearchMatch('preview 1', lineOneRange))]);

		// Add a file outside of the workspace folder (goes to "Other files")
		addToSearchResult(testObject, [aRawMatch('/other/outside.txt', new TextSearchMatch('preview 2', lineOneRange))]);
