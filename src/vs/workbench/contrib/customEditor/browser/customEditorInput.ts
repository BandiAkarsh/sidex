...
export class CustomEditorInput extends LazilyResolvedWebviewEditorInput {
	static create(
		...
		return instantiationService.invokeFunction(accessor => {
			...
			const input = instantiationService.createInstance(CustomEditorInput, init, webview, {
	...
	constructor(
		...
		super(
			{ providedId: init.viewType, viewType: init.viewType, name: '', iconPath: init.iconPath },
			webview,
			themeService,
	...
	override getName(): string {
		...
		this._editorName ??=
			this.customEditorLabelService.getName(this.resource) ?? basename(this.labelService.getUriLabel(this.resource));
	...
	public override matches(other: EditorInput | IUntypedEditorInput): boolean {
		...
		return (
			this === other ||
			(other instanceof CustomEditorInput && this.viewType === other.viewType && isEqual(this.resource, other.resource))
		);
	}
	...
	public override async resolve(): Promise<null> {
		...
		if (!this._modelRef) {
			...
			this._modelRef = this._register(
				assertReturnsDefined(await this.customEditorService.models.tryRetain(this.resource, this.viewType))
			);
