...
suite('Files - FileEditorInput', () => {
	...
	test('file editor serializer', async function () {
		...
		const editorSerializer = Registry.as<IEditorFactoryRegistry>(EditorExtensions.EditorFactory).getEditorSerializer(
			input.typeId
		);
		if (!editorSerializer) {
	...
	test('reports readonly changes', async function () {
		...
		try {
			accessor.fileService.readShouldThrowError = new NotModifiedSinceFileOperationError('file not modified since', {
				...stat,
				readonly: true
			});
			await input.resolve();
