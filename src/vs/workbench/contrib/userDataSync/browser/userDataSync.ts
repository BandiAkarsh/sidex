...
export class UserDataSyncWorkbenchContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		if (userDataSyncWorkbenchService.enabled) {
			...
			this._register(
				userDataSyncService.onDidChangeConflicts(() => this.onDidChangeConflicts(this.userDataSyncService.conflicts))
			);
			this._register(
			...
			this._register(
				Event.any(
					...
					() =>
						(this.turningOnSync =
							!userDataSyncEnablementService.isEnabled() && userDataSyncService.status !== SyncStatus.Idle)
				)
			);
		}
	}
					...
					const handle = this.notificationService.prompt(
						...
						[
							...
							},
							{
								...
								run: () => {
									this.telemetryService.publicLog2<{ source: string; action?: string }, SyncConflictsClassification>(
										'sync/showConflicts',
			...
			if (e instanceof UserDataSyncError) {
				switch (e.code) {
					...
					case UserDataSyncErrorCode.Forbidden:
						this.notificationService.error(
							localize('auth failed', 'Error while turning on Settings Sync: Authentication failed.')
						);
						return;
				}
				this.notificationService.error(
	...
	private async selectSettingsSyncService(userDataSyncStore: IUserDataSyncStore): Promise<void> {
		return new Promise<void>((c, e) => {
			...
			const quickPick = disposables.add(
				this.quickInputService.createQuickPick<{ id: UserDataSyncStoreType; label: string; description?: string }>()
			);
							...
							menu: [
								{
									...
									when: ContextKeyExpr.and(
										...
										ContextKeyExpr.regex(ctxMergeBaseUri.key, new RegExp(`^${USER_DATA_SYNC_SCHEME}:`))
									)
								}
							]
						});
