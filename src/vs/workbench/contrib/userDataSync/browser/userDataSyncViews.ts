			...
			registerAction2(
				class extends Action2 {
					...
					async run(accessor: ServicesAccessor, handle: TreeViewItemHandleArg): Promise<void> {
						...
						const { resource, comparableResource } = <{ resource: string; comparableResource: string }>(
...
class UserDataSyncMachinesViewDataProvider implements ITreeViewDataProvider {
	...
	async rename(machineId: string): Promise<boolean> {
		...
		const validateMachineName = (machineName: string): string | null => {
			...
			return machineName && !enabledMachines.some(m => m.id !== machineId && m.name === machineName)
				? machineName
				: null;
		};
		disposableStore.add(
