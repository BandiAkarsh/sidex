...
export class UserDataSyncConflictsViewPane extends TreeViewPane implements IUserDataSyncConflictsView {
	...
	private async getTreeItems(): Promise<ITreeItem[]> {
		...
		for (const previewResource of conflictResources) {
			let result =
				conflictResourcesByProfile[conflictResourcesByProfile.length - 1]?.[0].id === previewResource.profile.id
			...
			for (const resource of resources) {
				...
				const treeItem = {
					...
					command: {
						...
						title: '',
						arguments: [{ $treeViewId: '', $treeItemHandle: handle } satisfies TreeViewItemHandleArg]
					},
