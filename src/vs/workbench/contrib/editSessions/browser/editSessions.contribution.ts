...
export class EditSessionsContribution extends Disposable implements IWorkbenchContribution {
		...
		this._register(
			this.lifecycleService.onWillShutdown(e => {
				if (
					...
					this.configurationService.getValue('workbench.experimental.cloudChanges.autoStore') === 'onShutdown' &&
					!isWeb
				) {
				...
				class ContinueEditSessionAction extends Action2 {
					...
					async run(
						...
						if (!destination && !uri) {
							...
							if (!destination) {
								that.telemetryService.publicLog2<ContinueOnEventOutcome, ContinueOnClassificationOutcome>(
						...
						if (shouldStoreEditSession) {
							...
							that.telemetryService.publicLog2<ContinueWithEditSessionEvent, ContinueWithEditSessionClassification>(
									...
									async () => {
										...
										if (ref !== undefined) {
											that.telemetryService.publicLog2<ContinueOnEventOutcome, ContinueOnClassificationOutcome>(
												...
												{ outcome: 'storeSucceeded', hashedId: hashedEditSessionId(ref) }
											);
										} else {
											that.telemetryService.publicLog2<ContinueOnEventOutcome, ContinueOnClassificationOutcome>(
												'continueOn.editSessions.store.outcome',
				...
				class ResumeLatestEditSessionAction extends Action2 {
					...
					async run(
						...
						await that.progressService.withProgress(
							...
							async () => await that.resumeEditSession(editSessionId, undefined, forceApplyUnrelatedChange)
						);
					}
				}
			)
			...
			registerAction2(
				class ResumeLatestEditSessionAction extends Action2 {
					...
					async run(accessor: ServicesAccessor, editSessionId?: string): Promise<void> {
						...
						await that.progressService.withProgress(
							{ ...resumeProgressOptions, title: resumeProgressOptionsTitle },
							async () => await that.resumeEditSession(editSessionId, undefined, undefined, undefined, undefined, data)
						);
					}
				}
			)
	...
	async resumeEditSession(
		...
		if (!data) {
			if (ref === undefined && !silent) {
				...
				this.notificationService.warn(
					localize('no cloud changes for ref', 'Could not resume changes from the cloud for ID {0}.', ref)
				);
			}
			this.logService.info(
				ref !== undefined
		...
		if (editSession.version > EditSessionSchemaVersion) {
			...
			this.telemetryService.publicLog2<ResumeEvent, ResumeClassification>('editSessions.resume.outcome', {
			...
			for (const uri of trackedUris) {
				...
				if (await this.fileService.exists(uri)) {
					...
					if (editSessionSize > this.editSessionsStorageService.SIZE_LIMIT) {
						this.notificationService.error(
							localize('payload too large', 'Your working changes exceed the size limit and cannot be stored.')
						);
						return undefined;
					}
	...
	private registerContinueInLocalFolderAction(): void {
		...
		if (getVirtualWorkspaceLocation(this.contextService.getWorkspace()) !== undefined && isNative) {
			this.generateStandaloneOptionCommand(
				...
				localize('continueWorkingOn.existingLocalFolder', 'Continue Working in Existing Local Folder'),
				undefined,
				...
				quickPick.onDidTriggerItemButton(async e => {
					if (e.item.documentation !== undefined) {
						const uri = URI.isUri(e.item.documentation)
							...
							: await this.commandService.executeCommand<URI>(e.item.documentation);
						if (uri) {
	...
	private createPickItems(): (ContinueEditSessionItem | IQuickPickSeparator)[] {
		...
		const sortedItems: (ContinueEditSessionItem | IQuickPickSeparator)[] = items.sort((item1, item2) =>
