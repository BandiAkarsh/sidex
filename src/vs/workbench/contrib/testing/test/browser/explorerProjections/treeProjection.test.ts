...
suite('Workbench - Testing Explorer Hierarchal by Location Projection', () => {
	...
	test('expands children', async () => {
		...
		assert.deepStrictEqual(harness.flush(), [{ e: 'a', children: [{ e: 'aa' }, { e: 'ab' }] }, { e: 'b' }]);
	});
	...
	test('applies test changes (resort)', async () => {
		...
		assert.deepStrictEqual(harness.flush(), [{ e: 'a', children: [{ e: 'ab' }, { e: 'aa' }] }, { e: 'b' }]);
		...
		assert.deepStrictEqual(harness.flush(), [{ e: 'a', children: [{ e: 'a' }, { e: 'z' }] }, { e: 'b' }]);
		harness.pushDiff(
			{
				op: TestDiffOpType.Update,
		...
		assert.deepStrictEqual(harness.flush(), [{ e: 'a', children: [{ e: 'a2' }, { e: 'z2' }] }, { e: 'b' }]);
	});
	...
	test('fixes #204805', async () => {
		...
		harness.flush();
		harness.tree.expandAll();
		assert.deepStrictEqual(harness.tree.getRendered(), [{ e: 'a', children: [{ e: 'b', children: [{ e: 'c' }] }] }]);
	});
