	...
	getAttributes(port: number, host: string, commandLine?: string): Attributes | undefined {
		...
		while (index >= 0) {
			...
			if (found.key === port) {
				...
				attributes.elevateIfNeeded =
					attributes.elevateIfNeeded !== undefined ? attributes.elevateIfNeeded : found.elevateIfNeeded;
		...
		this._register(
			this.tunnelService.onTunnelOpened(async tunnel => {
				...
				if (
					...
					const attributes = (
						await this.getAttributes([{ port: tunnel.tunnelRemotePort, host: tunnel.tunnelRemoteHost }])
