...
class ViewDescriptorSequence {
	...
	constructor(model: IViewContainerModel) {
		...
		model.onDidAddVisibleViewDescriptors(
			added => added.forEach(({ viewDescriptor, index }) => this.elements.splice(index, 0, viewDescriptor)),
			null,
			this.disposables
		);
...
suite('ViewContainerModel', () => {
	...
	test('empty model', function () {
		...
		container = ViewContainerRegistry.registerViewContainer(
			{ id: 'test', title: nls.localize2('test', 'test'), ctorDescriptor: new SyncDescriptor(<any>{}) },
