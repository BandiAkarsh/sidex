...
suite('keyboardMapper - MAC de_ch', () => {
	...
	test('resolveKeybinding Cmd+K Cmd+\\', () => {
		_assertResolveKeybinding(KeyChord(KeyMod.CtrlCmd | KeyCode.KeyK, KeyMod.CtrlCmd | KeyCode.Backslash), [
			{
				label: '⌘K ⌃⇧⌥⌘7',
	...
	test('resolveKeybinding Cmd+K Cmd+=', () => {
		_assertResolveKeybinding(KeyChord(KeyMod.CtrlCmd | KeyCode.KeyK, KeyMod.CtrlCmd | KeyCode.Equal), [
			{
				label: '⌘K ⇧⌘0',
...
function _assertKeybindingTranslation(
	...
	const actual = actualHardwareKeypresses.map(k =>
		UserSettingsLabelProvider.toLabel(OS, [k], keybinding => ScanCodeUtils.toString(keybinding.scanCode))
	);
	assert.deepStrictEqual(
		actual,
		expected,
