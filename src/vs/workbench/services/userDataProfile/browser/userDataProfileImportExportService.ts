...
export class UserDataProfileImportExportService extends Disposable implements IUserDataProfileImportExportService {
			...
			.withProgress(
				...
				async progress => {
					const reportProgress = (message: string) =>
						progress.report({ message: localize('create from profile', 'Create Profile: {0}', message) });
	...
	private async resolveProfileContent(resource: URI): Promise<string | null> {
		...
		const context = await this.requestService.request(
			{ type: 'GET', url: resource.toString(true), callSite: 'userDataProfileImportExportService.resolveContent' },
			CancellationToken.None
		);
