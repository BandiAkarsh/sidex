...
function renderSuggestWidget(options: SuggestFixtureOptions): void {
	...
	const textModel = disposableStore.add(
		createTextModel(instantiationService, options.code, URI.parse('inmemory://suggest-fixture.ts'), 'typescript')
	);
	...
	suggestions: [
		{
			...
			range: { startLineNumber: 3, startColumn: 10, endLineNumber: 3, endColumn: 10 },
			detail: '(method) addEventListener(type: string, listener: EventListener): void'
		},
		{
			label: 'appendChild',
			kind: CompletionItemKind.Method,
