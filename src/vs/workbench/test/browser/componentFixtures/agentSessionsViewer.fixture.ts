...
function createMockSession(
	overrides: Partial<IAgentSession> & { label: string; status: AgentSessionStatus; providerType: string }
): IAgentSession {
	const now = Date.now();
	return new (class extends mock<IAgentSession>() {
		override readonly resource =
			overrides.resource ??
			URI.parse(`vscode-chat-session://${overrides.providerType}/session-${Math.random().toString(36).slice(2)}`);
