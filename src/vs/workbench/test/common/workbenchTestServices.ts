...
export function createFileStat(
	...
	children?:
		| { resource: URI; isFile?: boolean; isDirectory?: boolean; isSymbolicLink?: boolean; executable?: boolean }[]
		| undefined,
	executable?: boolean
): IFileStatWithMetadata {
	return {
		...
		children: children?.map(c =>
			createFileStat(c.resource, false, c.isFile, c.isDirectory, c.isSymbolicLink, undefined, c.executable)
		)
	};
}
...
export class TestFileService implements IFileService {
	...
	async resolveAll(toResolve: { resource: URI; options?: IResolveFileOptions }[]): Promise<IFileStatResult[]> {
		const stats = await Promise.all(
			toResolve.map(resourceAndOption => this.resolve(resourceAndOption.resource, resourceAndOption.options))
		);
